...
require_once '../model/ForgotPassword.php';
...
if (isset($_POST['method'])) {
    ...
    if(strcasecmp($_POST['method'],'userRegistration') == 0){
        $response['code'] = 1;
        $response['status'] = $api_response_code[$response['code']]['HTTP Response'];
        ...
        $objuserDetails->mapIncomingUserDetailsParams($name,$city,$mobileno,$email,$password);
    ...
    else if (strcasecmp($_POST['method'], 'login') == 0) {
        $response['code'] = 1;
        $response['status'] = $api_response_code[$response['code']]['HTTP Response'];
        $loginData = new UserLogin();
        $username = stripslashes($_POST['email']);
        $password = stripslashes($_POST['password']);
        $loginData->mapIncomingLoginParams($username, $password);
        $response['loginStatus'] = $loginData -> userLoginDetails();
        deliver_response($_POST['format'], $response, false);
    }
	else if(strcasecmp($_POST['method'],'checkemail') == 0){
        $response['code'] = 1;
        $response['status'] = $api_response_code[$response['code']]['HTTP Response'];
        $objuserDetails = new ForgotPassword();
        $email= stripslashes($_POST['email']);
        $response['checkemailResponse'] = $objuserDetails -> CheckingEmail($email);
        deliver_response($_POST['format'],$response,false);
    }  
	else if(strcasecmp($_POST['method'],'setNewPassword') == 0){
        $response['code'] = 1;
        $response['status'] = $api_response_code[$response['code']]['HTTP Response'];
        $objuserDetails = new ForgotPassword();
        $activationCode=stripslashes($_POST['code']);
